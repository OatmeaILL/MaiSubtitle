...
标点模型（中文补标点，0.3GB），加上依赖（含 PyTorch，约 3GB），合计约 8GB，首次安装比较慢。想先看缺什么、不下载：`安装_首次使用.bat --check`。
...
模型本来没装齐、又在设置里选了它，设置窗口底部会出现「下载缺失的模型」按钮，点一下会弹出一个命令窗口下载（实时进度在那个窗口里看，走镜像、可断点续传）。

想不知道选哪套搭配：设置 → 识别与翻译 → 「部署跑分（帮你选模型搭配）」。它对 3 种分句 ×
2 种识别 × 3 种翻译做全组合实测（4 语种切片，只跑本机已下载的模型），结束后在命令窗口里
直接给出排名与推荐（约 15~25 分钟，期间 GPU 会跑满）。

## 跑分参考（开发机实测）

跑分机器：**RTX 4060 Laptop GPU（8GB 显存）+ i7-13700H + 32GB 内存**，Windows 11。
切片：英语（BBC 播客 60s）/ 日语（TTS 43s）/ 韩语（游戏解说 60s）/ 中文（TTS 18s）。

![综合分排名](docs/bench_rank.svg)

- 推荐：**silero + whisper + qwen**（综合 83.1，识别 84.7，体感延迟 3.85s）；
  译得最快的是 silero + qwen3-asr + qwen（3.81s）。
- 识别模型对延迟影响很小（0.2~0.4s/段）；**延迟大头是等这句话说完（段长）**：
  silero 段最短所以最跟手，FireRed 句子更完整但要多等 1~2s，FSMN 会切出 10~20s 的
  超长段（垫底主因）。
- 翻译模型：qwen（0.16s/句）最快，hymt2（0.6~0.8s/句）译文自评最高但慢 3~4 倍。

![延迟构成](docs/bench_latency.svg)

你自己的机器跑一遍（结果和这套数据对照，就知道差距在显卡还是模型搭配）：

```
.venv\Scripts\python.exe scripts/bench_deploy.py
```

完整数据在 `docs/bench_dev.json`（本机结果，含每语种明细与打分理由）。
